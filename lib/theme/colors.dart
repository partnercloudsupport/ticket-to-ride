import 'package:flutter/material.dart';

// const kShrinePink50 = const Color(0xFFFEEAE6);
// const kShrinePink100 = const Color(0xFFFEDBD0);
// const kShrinePink300 = const Color(0xFFFBB8AC);
const ttrBlack400 = const Color(0xFF000000);
const ttrGold = const Color(0xFF3e2d21);

const playbackTurquoise400 = const Color(0xFF1AA6B7);

const playbackBlue400 = const Color(0xFF022D41);

// const kShrineBrown900 = const Color(0xFF442B2D);

const playbackErrorRed = const Color(0xFFC5032B);

const playbackSurfaceWhite = const Color(0xFFFFFBFA);
const playbackBackgroundWhite = Colors.white;
